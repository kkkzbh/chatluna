...
function _convertAgentStepToMessages(action, observation) {
  if (isToolsAgentAction(action) && action.toolCallId !== void 0) {
    ...
    return action.messageLog?.concat(
      new import_messages2.FunctionMessage(
        (0, import_string2.getMessageContent)(observation),
        action.tool
      )
    );
  } else {
...
function mergeHumanMessages(messages) {
  ...
  return new import_messages2.HumanMessage({
    content,
    name: base.name,
    id: base.id,
...
function createOpenAIAgent({
  ...
  const agent = import_runnables.RunnableSequence.from([
    ...
    import_runnables.RunnableLambda.from((input) => {
      ...
      const hasTools = input.additional_kwargs?.tool_calls?.length > 0 || (input instanceof import_messages2.AIMessageChunk || input instanceof import_messages2.AIMessage) && input.tool_calls?.length > 0;
      ...
      return outputParser.parseResult([
        {
          message: input,
          text: (0, import_string2.getMessageContent)(input.content)
        }
      ]);
    })
  ]);
...
async function* runAgent(options) {
  ...
  const config = signal == null ? options.config : (0, import_runnables2.patchConfig)(options.config, { signal });
    ...
    for await (const event of runner) {
      ...
      if (event.type === "done") {
        ...
        return {
          ...
          message: event.message ?? new import_messages3.AIMessage(event.output)
        };
      }
    }
...
function isAgentObservation(value) {
  ...
  return value.every((item) => (0, import_langchain.isMessageContentComplex)(item));
}
    ...
    [
      ...
      import_runnables3.RunnableLambda.from(async (input) => {
        const result = await outputParser.parse(
          (0, import_string3.getMessageContent)(input.content) ?? ""
        );

...
__export(message_exports, {
  DEFAULT_TOOL_MEMORY_MAX_ENTRIES: () => DEFAULT_TOOL_MEMORY_MAX_ENTRIES,
  INTERNAL_ADDITIONAL_ARG_PREFIX: () => INTERNAL_ADDITIONAL_ARG_PREFIX,
  KoishiChatMessageHistory: () => KoishiChatMessageHistory,
  TOOL_MEMORY_STORAGE_KEY: () => TOOL_MEMORY_STORAGE_KEY,
  buildToolMemoryEntriesFromSteps: () => buildToolMemoryEntriesFromSteps,
  parseToolMemoryEntries: () => parseToolMemoryEntries
});
...
function createAgentToolMessages(steps) {
  return [
    ...
    ...steps.map(
      (step) => new import_messages2.ToolMessage({
...
var KoishiChatMessageHistory = class extends import_chat_history.BaseChatMessageHistory {
  ...
  async addUserMessage(message) {
    const humanMessage = new import_messages2.HumanMessage(message);
    await this.addMessage(humanMessage);
  }
  async addAIChatMessage(message) {
    const aiMessage = new import_messages2.AIMessage(message);
  ...
  async normalizeResearchReplyHistory(finalVisibleText, updatedAt = /* @__PURE__ */ new Date()) {
      ...
      if (isConversationBoundaryRole(latestRole)) {
        ...
        if (normalizedText2.length > 0) {
          const normalizedMessage = await serializeMessage(
            new import_messages2.AIMessage(normalizedText2),
            this.conversationId,
            boundaryParentId
          );
    ...
    if (normalizedText.length > 0) {
      const normalizedMessage = await serializeMessage(
        new import_messages2.AIMessage(normalizedText),
        this.conversationId,
        boundaryParentId
      );
  ...
  async _loadMessages() {
    ...
    const promises = sorted.map(async (item) => {
      ...
      if (item.role === "system") {
        return new import_messages2.SystemMessage(fields);
      } else if (item.role === "human") {
        return new import_messages2.HumanMessage(fields);
      } else if (item.role === "ai") {
        return new import_messages2.AIMessage(fields);
      } else if (item.role === "function") {
        return new import_messages2.FunctionMessage(fields);
      } else if (item.role === "tool") {
        return new import_messages2.ToolMessage(fields);
      } else {

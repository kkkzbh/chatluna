...
async function countMessageTokens(message, tokenCounter) {
  ...
  let result = await tokenCounter((0, import_string.getMessageContent)(message.content)) + await tokenCounter((0, import_count_tokens.messageTypeToOpenAIRole)(message.getType()));
  if (message.name) {

...
async function countMessageTokens(message, tokenCounter) {
  ...
  let result = await tokenCounter(getMessageContent(message.content)) + await tokenCounter(messageTypeToOpenAIRole(message.getType()));
  if (message.name) {
...
async function countMessagesTokens(messages, tokenCounter) {
  let total = 0;
  ...
  return async (runtime, next) => {
    ...
    if (runtime.input) {
      ...
      const inputMessageForCount = {
        ...
        getType: /* @__PURE__ */ __name(() => input.getType(), "getType")
      };
...
import { BaseMessage as BaseMessage6 } from "@langchain/core/messages";
function createReadFilesContextMiddleware() {
  return async (context, next) => {
    const value = context.injection.value;
    const messages = Array.isArray(value) ? value : value instanceof BaseMessage6 ? [value] : [];
    if (messages.length === 0) {
      return next();
    }

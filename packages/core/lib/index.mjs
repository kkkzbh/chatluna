...
async function getFileConfig(ctx, model) {
  ...
  const [platform] = parseRawModelName4(model);
...
function apply39(ctx, config, chain) {
  ...
  chain.middleware("set_default_embeddings", async (session, context) => {
    ...
    const [, modelName] = parseRawModelName6(setEmbeddings);
...
import { parseRawModelName as parseRawModelName7 } from "koishi-plugin-chatluna/llm-core/utils/count_tokens";
  ...
  chain.middleware("test_model", async (session, context) => {
    ...
    try {
      if (!model.includes("/")) {
        ...
        ;
        [platformName, modelName] = parseRawModelName7(model);

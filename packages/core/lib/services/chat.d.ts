...
export declare class ChatLunaService extends Service<Config> {
     ...
     * @internal
     */
    chat(session: Session, room: ConversationRoom, message: Message, event: ChatEvents, stream?: boolean, variables?: Record<string, any>, postHandler?: PostHandler, requestId?: string, toolMask?: ToolMask): Promise<Message>;
    stopChat(room: ConversationRoom, requestId: string): Promise<boolean>;
    appendPendingMessage(conversationId: string, message: HumanMessage, chatMode?: string): false | Promise<boolean>;
...
declare class ChatInterfaceWrapper {
    ...
    chat(session: Session, room: ConversationRoom, message: Message, event: ChatEvents, stream: boolean, requestId: string, variables?: Record<string, any>, postHandler?: PostHandler, toolMask?: ToolMask): Promise<Message>;

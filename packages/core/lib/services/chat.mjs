...
var ChatChain = class {
  ...
  async _handleMiddlewareError(session, middlewareName, error) {
    if (error instanceof ChatLunaError) {
      ...
      if (error.errorCode === ChatLunaErrorCode.ABORTED && (state?.chatluna?.suppressAbortNotice === true || state?.qqReplyTransport?.suppressAbortNotice === true)) {
        return;
      }
...
function extractImages(content) {
  return Array.isArray(content) ? content.filter((item) => isMessageContentImageUrl(item)) : [];
}
...
function ensureMessageImageIntegrity(message) {
  ...
  throw new ChatLunaError3(
    ChatLunaErrorCode3.UNKNOWN_ERROR,
    new Error(
...
var ChatLunaService = class extends Service {
  ...
  async installPlugin(plugin) {
    ...
    if (this._plugins[platformName]) {
      throw new ChatLunaError3(
        ChatLunaErrorCode3.PLUGIN_ALREADY_REGISTERED,
   ...
   * @internal
   */
  chat(session, room, message, event, stream = false, variables = {}, postHandler, requestId = randomUUID(), toolMask) {
...
var ChatLunaPlugin = class {
  ...
  createRunnableConfig() {
    ...
    const abort = /* @__PURE__ */ __name(() => abortController.abort(
      new ChatLunaError3(ChatLunaErrorCode3.ABORTED, void 0, true)
    ), "abort");
...
function createAbortError() {
  return new ChatLunaError3(ChatLunaErrorCode3.ABORTED, void 0, true);
}
...
var ChatInterfaceWrapper = class {
  ...
  async chat(session, room, message, event, stream, requestId, variables = {}, postHandler, toolMask) {
    ...
    try {
      ...
      const humanMessage = new HumanMessage3({
